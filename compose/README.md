...
```kotlin
dependencies { 
    implementation("com.github.maxime-kouemo.EasySpans:compose:1.0.9") // temporary, will be moved to maven central
}
```
...
### Usage Example
Refer to the **Example 3: Clickable Text with Multiple addOccurrenceChunk and Uppercase Transformation** above which is an example of using `EasySpansClickableText` with `EasySpansCompose` to create clickable text with feedback via `Toast`.
